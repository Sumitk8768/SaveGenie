...
import { Search, Bell, AlertTriangle, Sparkles, Plus, Copy, Check } from 'lucide-react';
...
function CouponVaultPage() {
  const [couponList] = useState(INITIAL_COUPONS);
  const [activeTab, setActiveTab] = useState('Recent');
  const [searchQuery, setSearchQuery] = useState('');
  const [copiedId, setCopiedId] = useState(null);

  // Copy to clipboard core execution block
  const handleCopyCode = (id, codeString) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
        ...
        <div className="lg:col-span-7 space-y-5 order-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-xs font-ui">
                    ...
                    >
                      {copiedId === coupon.id ? (
                        <>
                          <Check className="h-3 w-3 text-[#6ffbbe]" />
                          <span>Copied!</span>
                        </>
                      ) : (
                        <>
                          <Copy className="h-3 w-3 text-zinc-400" />
                          <span className="font-mono text-zinc-300 tracking-normal">{coupon.code}</span>
                        </>
                      )}
                    </button>
            ...
            <button className="w-full py-5 rounded-2xl border border-dashed border-white/10 hover:border-white/20 hover:bg-white/[0.01] transition-all flex flex-col items-center justify-center gap-1.5 group cursor-pointer sm:col-span-2 lg:col-span-1">
